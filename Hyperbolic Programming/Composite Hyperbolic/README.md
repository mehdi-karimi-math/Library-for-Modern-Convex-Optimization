...
as a function $\lambda$ that returns the eigenvalues of $x$ with respect to the polynomial $e$. We have the following theorem for composing two hyperbolic polynomials:
...
by a $d$ by $m$ matrix where each row represents the coefficients of the variables in each term. For a given matrix $M$ and a direction $d$, 
...
`(c,A,b,cons)` to project $x_p$ on the hyperbolicity cone defined by the composite function. We can solve the problem as
